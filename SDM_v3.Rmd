...
```{r}
current <- dplyr::select(shapefile, c("Temp", "Chlor", "Mlost", "geometry" )) 
names(current) = c("temp_current", "chl_current", "mld_current", "geometry")
...
```{r}
predict_current = predict(train_fit_ws, current, type = "prob") %>% #gets the testing prediction
  bind_cols(current) %>%
  mutate(check = .pred_ABSENT + .pred_PRESENT) %>% 
  mutate(pred_binary = ifelse(.pred_PRESENT >= 0.75, "1", "0"))
...
```{r}
world = st_read(here::here("Gulf_of_Mexico_uncropped"))
...
world_subset <- st_intersection(world, bounding_box)
...
#gpt code
matrix_ws <- matrix(c(-98.86, 14.37,
                   -80.63, 14.37,
                   -80.63, 31.14,
                   -98.86, 31.14,
                   -98.86, 14.37), ncol = 2, byrow = TRUE)

polybox <- st_polygon(list(matrix_ws)) %>% 
  st_sfc(crs = st_crs(world))
  
gulf <- st_intersection(polybox, world)
```

SDM woooo
```{r}
predict_current = st_as_sf(predict_current)
predict_ssp2_2050 = st_as_sf(predict_ssp2_2050)
predict_ssp2_2090 = st_as_sf(predict_ssp2_2090)
predict_ssp4_2050 = st_as_sf(predict_ssp4_2050)
predict_ssp4_2090 = st_as_sf(predict_ssp4_2090)
```
...
```{r}
current <- ggplot() +
  geom_sf(data = predict_current, aes(color = .pred_PRESENT)) +
  #geom_sf(data = world) +
  theme_bw() +
...
sdm2_50 <- ggplot() +
  geom_sf(data = predict_ssp2_2050, aes(color = .pred_PRESENT)) +
  #geom_sf(data = gulf) +
  theme_bw() +
  scale_color_viridis_c() +
  labs(color = "Probability")+
...
sdm4_50 <- ggplot() +
  geom_sf(data = predict_ssp4_2050, aes(color = .pred_PRESENT)) +
  #geom_sf(data = gulf) +
  theme_bw() +
  scale_color_viridis_c() +
  labs(color = "Probability")+
...
sdm2_90 <- ggplot() +
  geom_sf(data = predict_ssp2_2090, aes(color = .pred_PRESENT)) +
  #geom_sf(data = gulf) +
  theme_bw() +
  scale_color_viridis_c() +
  labs(color = "Probability")+
...
sdm4_90 <- ggplot() +
  geom_sf(data = predict_ssp4_2090, aes(color = .pred_PRESENT)) +
  #geom_sf(data = gulf) +
  theme_bw() +
  scale_color_viridis_c() +
  labs(color = "Probability")+
...
```{r}
#current + 
#sdm2_50 + sdm4_50 + plot_spacer()+ 
sdm2_90 + sdm4_90 + plot_layout(guides = 'collect')
```

```{r}
mean(predict_current$.pred_PRESENT)
mean(predict_ssp2_2050$.pred_PRESENT)
mean(predict_ssp4_2050$.pred_PRESENT)
mean(predict_ssp2_2090$.pred_PRESENT)
mean(predict_ssp4_2090$.pred_PRESENT)

mean(predict_current$temp_current)
mean(predict_ssp2_2050$temp_current)
mean(predict_ssp4_2050$temp_current)
mean(predict_ssp2_2090$temp_current)
mean(predict_ssp4_2090$temp_current)

mean(predict_current$chl_current)
mean(predict_ssp2_2050$chl_current)
mean(predict_ssp4_2050$chl_current)
mean(predict_ssp2_2090$chl_current)
mean(predict_ssp4_2090$chl_current)

mean(predict_current$mld_current)
mean(predict_ssp2_2050$mld_current)
mean(predict_ssp4_2050$mld_current)
mean(predict_ssp2_2090$mld_current)
mean(predict_ssp4_2090$mld_current)

cor.test(predict_current$temp_current, predict_current$.pred_PRESENT)
cor.test(predict_ssp2_2050$temp_current, predict_ssp2_2050$.pred_PRESENT)
cor.test(predict_ssp4_2050$temp_current, predict_ssp4_2050$.pred_PRESENT)
cor.test(predict_ssp2_2090$temp_current, predict_ssp2_2090$.pred_PRESENT)
cor.test(predict_ssp4_2090$temp_current, predict_ssp4_2090$.pred_PRESENT)

write.csv(predict_current, "Present Predictions.csv")
write.csv(predict_ssp2_2050, "ssp2 2050 Predictions.csv")
write.csv(predict_ssp4_2050, "ssp4 2050 Predictions.csv")
write.csv(predict_ssp2_2090, "ssp2 2090 Predictions.csv")
write.csv(predict_ssp4_2090, "ssp4 2090 Predictions.csv")
```


```{r}
test_roc_acu_value = roc_auc(test_predict2_whaleshark, occurance_status, .pred_ABSENT)
